...
- `/clock` публикует `Ros2Supervisor` из `webots_ros2_driver`. Интерфейсы из [INTERFACES.md](../INTERFACES.md) не меняются.
- Первая строка каждого мира — `#VRML_SIM R2025a utf8`. Миры сохраняются только из Webots R2025a, стандартные модели подключаются через `webots://`, собственные PROTO и текстуры хранятся в репозитории. Стартовая проверка отклоняет мир, если Webots печатает `ERROR:`.
